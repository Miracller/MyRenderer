...
template<typename T, size_t NROW, size_t NCOL> struct Matrix 
{
	...
	Matrix<T, NROW, NCOL> operator*(const Matrix<T, NROW, NCOL>&) const; //*(乘号)的操作要返回拷贝对象，不是在原对象上修改所以没有&
...
// different dimensions of matrix multiplication and vector multiplication
...
template<typename T, size_t NROW, size_t NCOL>
	...
	return minor(row, col).det() * ( ((row + col) % 2 == 1) ? -1 : 1 );
}
...
// 每个位置变为该位置的代数余子式，所得到的矩阵"转置"便是伴随矩阵
// 伴随矩阵 A* = C^T
template<typename T, size_t NROW, size_t NCOL>
...
Matrix<T, NROW, NCOL> Matrix<T, NROW, NCOL>::inverse_transpose(){
	Matrix<T, NROW, NCOL> ret = inverse();
